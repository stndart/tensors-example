                ...
                for (size_t ci = 0; ci < C; ++ci)
                    for (size_t hi = 0; hi < H; ++hi)
                        for (size_t wi = 0; wi < W; ++wi) {
                            ...
                            __half candidate = 0;
                            if (idx >= 0 &&
                                idx < input.vsize()) // Zero padding branch
                                candidate = input[idx];

                            if (candidate > output[{bi, ci, ohi, owi}]) {
                                output[{bi, ci, ohi, owi}] = candidate;
                                argmax_cache_h[{bi, ci, ohi, owi}] = fhi;
                                argmax_cache_w[{bi, ci, ohi, owi}] = fwi;
                            }
                        }
        }
}
        ...
        for (size_t owi = 0; owi < output_gradient.dimZ(); ++owi) {
            ...
            for (size_t bi = 0; bi < B; ++bi)
                for (size_t ci = 0; ci < C; ++ci)
                    for (size_t hi = 0; hi < H; ++hi)
                        for (size_t wi = 0; wi < W; ++wi) {
                            int fhi = argmax_cache_h[{bi, ci, ohi, owi}];
                            int fwi = argmax_cache_w[{bi, ci, ohi, owi}];
