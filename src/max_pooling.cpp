...
MaxPooling::MaxPooling(Index4 input_shape, PaddingMode padding_mode,
    ...
    : argmax_cache_h(Index4()), argmax_cache_w(Index4()),
    ...
    size_t output_H = (input_shape.y + 2 * H_pad_ - H_size_) / H_stride_ + 1;
    size_t output_W = (input_shape.z + 2 * W_pad_ - W_size_) / W_stride_ + 1;

    argmax_cache_h = Tensor4D(input_shape.w, input_shape.x, output_H, output_W);
    argmax_cache_h.allocate_memory();
    argmax_cache_h.fill(0);
    argmax_cache_w = Tensor4D(input_shape.w, input_shape.x, output_H, output_W);
    argmax_cache_w.allocate_memory();
    argmax_cache_w.fill(0);
}

void MaxPooling::forward(const Tensor4D &input, Tensor4D &output) {
    if (output.vsize() != argmax_cache_h.vsize())
...
void MaxPooling::backward(const Tensor4D &output_gradient,
    ...
    if (output_gradient.vsize() != argmax_cache_h.vsize())
        throw std::runtime_error("Backward: output dimensions mismatch");

#ifdef USE_CUDA
    try {
        cuda_max_pooling_backward(output_gradient, input_gradient, argmax_cache_h, argmax_cache_w, padding, H_pad_,
                         W_pad_, H_stride_, W_stride_, H_size_, W_size_);
        return;
    } catch (const std::exception &e) {
        std::cerr << "CUDA error: " << e.what();
    ...
    cpu_max_pooling_backward(output_gradient, input_gradient, argmax_cache_h, argmax_cache_w, padding,
                    H_pad_, W_pad_, H_stride_, W_stride_, H_size_, W_size_);
}