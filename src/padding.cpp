...
__half padded_access(const TensorType &tensor, typename TensorType::Index idx, PaddingMode mode) {
    if (idx < tensor.vsize())
        return tensor[idx];
    ...
    case PaddingMode::ZERO_PADDING:
        return static_cast<__half>(0.0f);
...
template __half padded_access<Tensor4D>(const Tensor4D &tensor, Index4 idx,
                                        PaddingMode mode);
template __half padded_access<Matrix>(const Matrix &tensor, Index2 idx,
                                      PaddingMode mode);